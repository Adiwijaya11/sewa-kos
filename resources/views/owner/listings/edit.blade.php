...
@section('title', 'Ubah Kos - KosinAja')
@section('header_title', 'Ubah Detail Kos')

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="kosEditForm()">
    <!-- Header Card -->
    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
        <h2 class="text-base font-bold text-slate-800">Ubah Properti Kos: {{ $listing->title }}</h2>
        <p class="text-xs text-slate-400 mt-1">Ubah rincian kos Anda. Setiap perubahan pada kos non-verified akan tetap diulas oleh Admin.</p>
    </div>
    ...
    <form action="{{ route('owner.listings.update', $listing->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                ...
                <div class="space-y-1.5 sm:col-span-2 md:col-span-3">
                    ...
                    <input type="text" name="title" value="{{ old('title', $listing->title) }}" placeholder="Contoh: Kos Putri Eksklusif Menteng Indah tipe A" 
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="price" value="{{ old('price', $listing->price) }}" placeholder="Contoh: 1500000" min="10000"
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="room_size" value="{{ old('room_size', $listing->room_size) }}" placeholder="Contoh: 3x4 m atau 4x4" 
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="max_people" value="{{ old('max_people', $listing->max_people) }}" placeholder="Contoh: 1" min="1" max="50"
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="total_rooms" value="{{ old('total_rooms', $listing->total_rooms ?? 10) }}" placeholder="Contoh: 10" min="1" max="1000"
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="available_rooms" value="{{ old('available_rooms', $listing->available_rooms ?? 5) }}" placeholder="Contoh: 5" min="0" max="1000"
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="gender_type" class="w-full h-10 px-3 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs capitalize" required>
                        ...
                        @foreach(\App\Models\Setting::getGenderTypes() as $type)
                            <option value="{{ $type }}" {{ old('gender_type', $listing->gender_type) === $type ? 'selected' : '' }}>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="5" placeholder="Tuliskan spesifikasi kamar, akses kunci 24 jam, kebijakan listrik, dan informasi pelengkap lainnya..." 
                          class="w-full p-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>{{ old('description', $listing->description) }}</textarea>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="province" id="provinceInput" value="{{ old('province', $listing->province) }}" placeholder="Contoh: DKI Jakarta" 
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="city" id="cityInput" value="{{ old('city', $listing->city) }}" placeholder="Contoh: Jakarta Selatan" 
                           class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>
                </div>
                ...
                <div class="space-y-1.5 md:col-span-3">
                    ...
                    <textarea name="address" rows="2" placeholder="Contoh: Jl. Menteng Raya No. 45, RT 02/RW 03, Menteng" 
                              class="w-full p-3 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs" required>{{ old('address', $listing->address) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 gap-3 mb-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div>
                        ...
                        <input type="text" name="latitude" id="latitudeInput" value="{{ old('latitude', $listing->latitude) }}" readonly 
                    ...
                    <div>
                        ...
                        <input type="text" name="longitude" id="longitudeInput" value="{{ old('longitude', $listing->longitude) }}" readonly 
                               class="text-xs font-bold text-slate-700 bg-transparent border-none p-0 focus:ring-0 select-all cursor-default">
                    </div>
                </div>
            ...
            <div class="border-t border-slate-100 pt-5 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="near_campus" value="{{ old('near_campus', $listing->near_campus) }}" placeholder="Contoh: Kampus UGM (5 menit jalan kaki)" 
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="near_mall" value="{{ old('near_mall', $listing->near_mall) }}" placeholder="Contoh: Ambarrukmo Plaza (10 menit berkendara)" 
                               class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="near_hospital" value="{{ old('near_hospital', $listing->near_hospital) }}" placeholder="Contoh: RS Sardjito (6 menit)" 
                               class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="near_station" value="{{ old('near_station', $listing->near_station) }}" placeholder="Contoh: Stasiun Tugu (10 menit)" 
                               class="w-full h-10 px-3.5 rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-500 text-xs">
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                @foreach($facilities as $facility)
                    @php
                        $hasFacility = $listing->facilities->contains($facility->id);
                    @endphp
                    <label class="flex items-center space-x-3.5 p-3 rounded-xl border border-slate-100 hover:border-emerald-100 bg-white hover:bg-slate-50/50 cursor-pointer shadow-sm transition-all select-none">
                        <input type="checkbox" name="facilities[]" value="{{ $facility->id }}" 
                               {{ (is_array(old('facilities')) && in_array($facility->id, old('facilities'))) || $hasFacility ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-5">
            ...
            @if($listing->images->isNotEmpty())
                <div class="space-y-2">
                    <span class="text-xs font-bold text-slate-700 block">Foto Kos Saat Ini:</span>
                    <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3.5 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                        @foreach($listing->images as $img)
                            <div class="relative group aspect-square rounded-xl overflow-hidden border border-slate-200 bg-white shadow-sm">
            ...
            <div class="space-y-4">
                ...
                <label class="border-2 border-dashed border-slate-200 hover:border-emerald-500 rounded-2xl p-6 bg-slate-50/50 hover:bg-emerald-50/10 flex flex-col items-center justify-center cursor-pointer transition-all relative">
                    ...
                    <span class="text-xs font-bold text-slate-700">Unggah Tambahan Foto Baru</span>
                ...
                <div x-show="previews.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5 p-3.5 bg-slate-50 rounded-2xl border border-slate-100" x-cloak>
                    <template x-for="(img, idx) in previews" :key="img.id">
                        <div class="relative group aspect-square rounded-xl overflow-hidden border border-slate-200 bg-white shadow-sm transition-all hover:scale-[1.02]">
                            ...
                            <!-- Index Badge -->
                            <span class="absolute top-2 left-2 bg-slate-900/70 backdrop-blur-sm text-white text-[9px] font-bold px-1.5 py-0.5 rounded-lg" x-text="'Foto baru ' + (idx + 1)"></span>
        ...
        <div class="flex items-center space-x-3 justify-end pt-2">
            ...
            <button type="submit" class="px-6 h-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-all flex items-center justify-center shadow-lg shadow-emerald-600/20">
                <i class="fa-solid fa-cloud-arrow-up mr-2"></i>Simpan Perubahan Kos
            </button>
        </div>
    </form>
</div>
...
<script>
    ...
    window.addEventListener('load', () => {
        // Fetch existing coordinates or use default
        const initLat = {{ $listing->latitude ?? -6.2088 }};
        const initLng = {{ $listing->longitude ?? 106.8456 }};

        document.getElementById('latitudeInput').value = parseFloat(initLat).toFixed(6);
        document.getElementById('longitudeInput').value = parseFloat(initLng).toFixed(6);

        const map = L.map('mapPicker').setView([initLat, initLng], 12);
        ...
        gmapsInput.addEventListener('input', function (e) {
            ...
            if (val.startsWith('http') || val.includes(',')) {
                feedback.textContent = `⚠ Koordinat tidak terbaca. Pastikan link Google Maps berisi koordinat atau paste koordinat mentah (contoh: -6.2088, 106.8456)`;
